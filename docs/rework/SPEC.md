...
- **Audit pagination**: the shared read module validates requests before reading state. Empty cursor starts the first page; supplied cursors contain only decimal digits and represent a positive signed 64-bit integer. Omitted, zero, or negative limits use 50; malformed limits and pagination overflow are rejected. CLI and MCP report `usage`; both Web UI audit routes honor cursors and return HTTP 400 for invalid pagination. Covered by `TestAuditRejectsMalformedRequestsBeforeReadingState`, `TestAuditContinuesFilteredPagesAndReturnsNullOnTheLastPage`, `TestAuditRejectsInvalidCursorsAsUsageErrors`, `TestTheAuditToolRejectsInvalidCursorsAsUsageErrors`, `TestTheAuditPageRejectsInvalidCursorsWithBadRequest`, and `TestTheAuditAPIContinuesFromItsNextCursor`.
