...
3. Store the user's Portainer API key in `secrets/portainer-api-key` with mode
   `600`.
4. Create the private external Docker networks referenced by the Compose file
   and attach Portainer plus the health-checked application as appropriate.
5. Start with `docker compose -f compose.monitor.yaml up --build -d` and inspect
...
state database, certificate, Compose environment secret, or private host data.
